...
class Round {
  constructor(roundString) {

    this.blue = 0;
    this.green = 0;
    this.red = 0;

    let colors = roundString.split(', ')
...
const partOne = (input) => {
  ...
  input = input.map( (line) => {
    ...
    roundsArr = [];
    for(round of rounds) {
      roundsArr.push(new Round(round))
    }
    
    return new Game(id, roundsArr)
  })
  ...
  for(let game of input) {
    ...
    for(let round of game.rounds){
      let {blue, red, green} = round.getColors();
    ...
    if(possible) possibleGames.push(+game.id);
  }

  return possibleGames.reduce( (curr, total) => curr + total)
}
...
const partTwo = (input) => {
  ...
  return powers.reduce( (curr, total) => curr + total)
}
